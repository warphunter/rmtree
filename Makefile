...
$(BINWIN64): $(SRC) $(INC)
	...
	set -x; \
	x86_64-w64-mingw32-gcc -D__USE_MINGW_ANSI_STDIO -DCC_USED="\"`x86_64-w64-mingw32-gcc --version|head -1`\"" $(CFLAGSWIN) $(SRC) $(SRCWINX) -o $@ -lpthread -static; \
	set +x; \
...
$(BINWIN32): $(SRC) $(INC)
	...
	set -x; \
	i686-w64-mingw32-gcc -D__USE_MINGW_ANSI_STDIO -DCC_USED="\"`i686-w64-mingw32-gcc --version|head -1`\"" $(CFLAGSWIN) $(SRC) $(SRCWINX) -o $@ -lpthread -static; \
	set +x; \
	test -f $(BINWIN32) && mv -f $(BINWIN32) ..

test:
	@ksh=`which ksh`; \
	if [ "$$ksh" = "" ]; then echo Note: This test won\'t run since you don\'t have ksh installed or not in your PATH.; exit 0; elif [ "$$ksh" != /bin/ksh ]; then ksh ./test$(BIN) -c; else ./test$(BIN) -c; fi

realtest:
	@ksh=`which ksh`; \
	if [ "$$ksh" = "" ]; then echo Note: This test won\'t run since you don\'t have ksh installed or not in your PATH.; exit 0; elif [ "$$ksh" != /bin/ksh ]; then ksh ./mktree; $(BIN) -S .a .b .c; else ./mktree; $(BIN) -S .a .b .c; fi
...
.PHONY : all win64 win32 win test realtest install uninstall clean
